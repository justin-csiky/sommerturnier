...
def draw_tree(klasse, number_groups):
    ...
    if c.execute("SELECT id, name FROM teams WHERE class=? and quaters_nr_winner=2",(klasse,)).fetchone():
        semi1_team2 = c.execute("SELECT id, name FROM teams WHERE class=? and quaters_nr_winner=2",(klasse,)).fetchone()
    if c.execute("SELECT id, name FROM teams WHERE class=? and quaters_nr_winner=3",(klasse,)).fetchone():
        semi2_team1 = c.execute("SELECT id, name FROM teams WHERE class=? and quaters_nr_winner=3",(klasse,)).fetchone()
    if c.execute("SELECT id, name FROM teams WHERE class=? and quaters_nr_winner=4",(klasse,)).fetchone():
        semi2_team2 = c.execute("SELECT id, name FROM teams WHERE class=? and quaters_nr_winner=4",(klasse,)).fetchone()


    if c.execute("SELECT id, name FROM teams WHERE class=? and semis_nr_winner=1",(klasse,)).fetchone():
        ...
        with st.container(horizontal=True):
            ...
            with st.container(width=500):
                ...
                with grA:
                    ...
                    with st.container(horizontal=True):
                        st.space("stretch")
                        if c.execute("SELECT id, class FROM groups WHERE is_done=0 and class='LVL1/2' and group_name='A'").fetchone():
                ...
                with grB:
                    ...
                    with st.container(horizontal=True):
                        st.space("stretch")
                        if c.execute("SELECT id, class FROM groups WHERE is_done=0 and class='LVL1/2' and group_name='B'").fetchone():
                ...
                with grC:
                    ...
                    with st.container(horizontal=True):
                        st.space("stretch")
                        if c.execute("SELECT id, class FROM groups WHERE is_done=0 and class='LVL1/2' and group_name='C'").fetchone():
